...
enabled:
- `ACCELEROMETER_MEASURE CHIP=<config_name> [RATE=<value>]`: Starts
  accelerometer measurements at the requested number of samples per
  second. Valid rates are 25, 50, 100, 200, 400, 800, 1600, and 3200.
  If RATE is zero (or not specified) then the current series of
  measurements are stopped and the results are written to a file named
  `/tmp/adxl345-YYYYMMDD_HHMMSS.csv`.
